...
describe('Amphibian species tests', () => {
  test('After an expansion on a wetland if the player has a Amphibian species, game should proceed to the DrawCubes rule for the current player and allow to draw 1 cube', () => {
    ...
    const { setup, game } = setupGameAndGetColonizationRule(
      ...
      [{ cardId: SpeciesCard.GreatCrestedNewt, cubeBiotopeType: BiotopeType.Wetland }],
  ...
  test('After an expansion on a wetland if the player has a Amphibian species and player drew a cube, game should proceed to the ChooseAction rule for the next player', () => {
    // Given
  ...
  test('After a migration to a wetland if the player has a Amphibian species and player drew a cube, game should proceed to the ChooseAction rule for the next player', () => {
    // Given
    const { setup, game, cardsWithIndexes } = setupGameAndGetColonizationRule(
      [PlayerColor.Ibex, PlayerColor.Fox],
      [
