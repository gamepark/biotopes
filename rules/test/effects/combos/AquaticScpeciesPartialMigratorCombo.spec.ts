...
describe('Cosmopolitan and Aquatic species combo tests', () => {
  test('Given an expansion on a wetland for a player having a partial migrator and an aquatic species, game should proceed to the DrawCard rule for the current player and allow to draw 1 card', () => {
    const { setup, game } = setupGameAndGetColonizationRule(
      [PlayerColor.Woodpecker, PlayerColor.Owl],
      [
    ...
    playAction(
      rules,
      rules.material(MaterialType.TerritoryToken).player(PlayerColor.Woodpecker).moveItem({
    ...
    // Then
    expect(rules.rulesStep).to.be.instanceof(DrawCardsRule).and.have.property('player').which.eql(PlayerColor.Woodpecker)
  ...
  test('Given a migration to a wetland for a player having a partial migrator and an aquatic species, game should proceed to the DrawCard rule for the current player and allow to draw 2 cards', () => {
