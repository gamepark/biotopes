...
describe('Cosmopolitan and Forest species combo tests', () => {
  test('Given an expansion on a forest for a player having a partial migrator and a forest species, game should proceed to the DrawCard rule for the current player and allow to draw 1 cards', () => {
  ...
  test('Given a migration to a forest for a player having a partial migrator and a forest species, game should proceed to the DrawCard rule for the current player and allow to draw 1 cards', () => {
    const { game } = setupGameAndGetColonizationRule(
      [PlayerColor.Salamander, PlayerColor.Woodpecker],
      [
        { cardId: SpeciesCard.StagBeetle, cubeBiotopeType: BiotopeType.Forest },
